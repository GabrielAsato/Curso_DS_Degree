...
--- Insert Table employee_territories
...
-- da França e da Espanha, excluindo-se os clientes que vivem nas capitais destes países.
...
-- Na tabela Products:
-- 3) Quantos são os produtos que são da CategoryID 2?
SELECT COUNT (*) FROM products WHERE  category_id = 2;

-- 4) Quantos produtos com SupplierID idêntico ao CategoryID e que custam mais do que 19?
SELECT COUNT(*) FROM products WHERE supplier_id = category_id AND unit_price > 19;

-- 5) Liste todos os nomes de produtos, suas *"Units"* e seus respectivos preços distintos 
...
-- 6) Liste os 5 primeiros nomes de produtos que começam com "A" ou tenham "h" no meio do nome.
