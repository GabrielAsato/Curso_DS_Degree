...
-----EXERCÍCIOS--Exercícios de Álgebra Relacional
...
FROM funcionario 
WHERE (datanasc BETWEEN '1961/01/01' AND '1970/12/31') AND
sexo = 'F' AND
salario < 1000;

--3. Selecione o nome dos dependentes do empregado de nome ‘Luciana S. Santos’.
-- Todos os funcionarios com e sem dependentes
SELECT * FROM funcionario
LEFT JOIN dependente ON funcionario.id_func = dependente.id_func;

SELECT nomedep FROM dependente
INNER JOIN funcionario ON dependente.id_func = funcionario.id_func
WHERE funcionario.nomefunc = 'Luciana S. Santos'; -- sem dependentes
...
FROM funcionario
WHERE EXISTS (
	SELECT 1 FROM dependente WHERE funcionario.id_func = dependente.id_func
);
-- é similar a
