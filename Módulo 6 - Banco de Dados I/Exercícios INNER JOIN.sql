...
-- Como usar a função
SELECT SUM((products.unit_price * order_details.quantity * order_details.discount)) + AVG(orders.freight)
AS valor_total FROM products, order_details, orders;

SUM((products.unit_price * order_details.quantity * (1 - order_details.discount))) + AVG(orders.freight) 
AS total_order_price

ROUND(SUM((products.unit_price * order_details.quantity * (1 - order_details.discount))) + AVG(orders.freight))
AS total_order_price

(SUM((products.unit_price * order_details.quantity * (1 - order_details.discount))) + AVG(orders.freight)) / COUNT(orders.order_id)
AS mean_order_price
-------------------------------------------------------------------------------------------------------
